...
public class QueryViewController {
  ...
  private static final List<String> SELECT_FORMATS =
      List.of("XML", "JSON", "CSV", "TSV", "MARKDOWN");
  ...
  private void initializeTopBar() {
    ...
    topBar
        .getButton(IconButtonType.TEMPLATE)
        .setOnAction(
            e -> {
              Stage stage = (Stage) mainBorderPane.getScene().getWindow();
                        ...
                        if (tab != tabEditorController.getView().getAddTab()) {
                          Platform.runLater(
                              () -> {
                                ...
                                if (controller != null) configureEditorRunButton(controller);
                              });
                        }
                      }
                    }
  ...
  private void setupLayout() {
    Platform.runLater(
        () -> {
          if (mainSplitPane != null) mainSplitPane.setDividerPosition(0, 0.6);
        });
  }
