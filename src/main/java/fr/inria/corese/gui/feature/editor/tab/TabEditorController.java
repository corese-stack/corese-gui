...
public class TabEditorController {
	 ...
	 * {@link TabEditorConfig#shouldPreloadFirstTab()} is enabled. It remains in
	 * memory but invisible until the user creates a new empty tab, at which point
	 * this preloaded instance is reused for instant display.
	 *
	 * <p>
	 ...
	 * This ensures the controller is always in a valid state and prevents
	 * incomplete initialization.
	 *
	 * @param config
	 *            The configuration for this controller (must not be null)
	...
	/**
	 * Opens accepted dropped files and reports ignored ones.
	 *
	 * <p>
	 * File eligibility is delegated to {@link TabEditorDropFilePolicy}.
	 */
	private void handleDroppedFiles(List<File> droppedFiles) {
		TabEditorDropFilePolicy.DropEvaluation evaluation = dropFilePolicy.evaluate(droppedFiles);
	...
	private Runnable resolveOpenFileAction() {
		...
		Optional<TabEditorConfig.MenuItem> fallback = config.getMenuItems().stream().filter(item -> item.text() != null)
				.filter(item -> item.text().toLowerCase().contains("open")).findFirst()
	 ...
	 * empty content), the preloaded tab is reused for instant display. Otherwise, a
	 * new tab is created.
	 *
	 * @param title
	 *            The tab title
	 ...
	 * unsaved changes, shows a confirmation dialog.
	 *
	 * @param tab
	 *            The tab to close
	 */
	public void requestCloseTab(Tab tab) {
		...
		view.showUnsavedChangesDialog(controller.getModel().getDisplayName(), result -> {
			switch (result) {
	 ...
	 * needed, typically when the application is closing.
	 *
	 * <p>
	 * <b>Note:</b> File loading threads are daemon threads managed by
	 * {@link ImportHelper} and will automatically terminate when the application
	 * exits.
	 */
	 ...
	 *            Initial content
	 * @param filePath
	 *            File path (null if not associated with a file)
	 ...
	 * instant display when needed later.
	 *
	 * @param title
	 *            Tab title
	 * @param content
	 ...
	 *            The execution button (nullable)
	 */
	private void attachContext(Tab tab, CodeEditorController editorController, ResultController resultController,
	...
	private FloatingButtonWidget setupExecutionButton(StackPane editorWrapper) {
	...
	private void bindTabProperties(Tab tab, CodeEditorController editorController) {
		...
		editorController.getModel().modifiedProperty()
		...
		if (context != null && context.hasExecutionButton()) {
			...
			context.getExecutionButton().disableProperty().bind(isEmpty.or(context.executionRunningProperty()));
		}
	}
	...
	private void startFileLoadingTask(Tab tab, File file) {
		ImportHelper.loadFileAsync(file, content -> {
			unlockTabUI(tab);
	 ...
	 * CodeEditorController (unbinding CodeMirror content and cleaning up listeners)
