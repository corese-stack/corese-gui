 ...
 *
 * <p>
 * All direct Corese API usage for conversion should be isolated here to keep
...
public final class RdfConversionService {
	 ...
	 *
	 * @param content
	 *            The RDF content to convert
	 * @param sourceFormat
	 *            The source serialization format
	 * @param targetFormat
	 *            The target serialization format
	 * @return The converted content as a string
	 */
	public String convertGraphContent(String content, SerializationFormat sourceFormat,
			SerializationFormat targetFormat) {
		if (content == null) {
			throw new IllegalArgumentException("Content cannot be null.");
		}
		Graph graph = parseGraphContent(content, sourceFormat);
		try {
			return ResultFormatter.getInstance().formatGraphOrThrow(graph, targetFormat);
		} catch (ResultFormatter.ResultFormattingException e) {
			throw new IllegalStateException("Failed to format graph to " + targetFormat + ": " + e.getMessage(), e);
		}
	}
