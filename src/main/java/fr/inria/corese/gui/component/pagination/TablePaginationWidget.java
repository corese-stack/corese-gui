...
public class TablePaginationWidget extends HBox {
	 ...
	 *
	 * @param onPageChange
	 *            Callback invoked when the page changes. Receives the new page
	 *            index (0-based).
	 */
	public TablePaginationWidget(Consumer<Integer> onPageChange) {
		...
		this.prevButton = new IconButtonWidget(ButtonFactory.custom(ButtonIcon.PREVIOUS_PAGE, "Previous Page",
	 ...
	 *
	 * @param pageCount
	 *            The total page count (minimum 1).
	 */
	 ...
	 *
	 * @param index
	 *            The page index to display (0-based).
	 */
	 ...
	 *
	 * @param text
	 *            The text to display (e.g., "Total rows: 500").
	 */
	 ...
	 *
	 * @param value
	 *            The number of rows per page as a string.
	 */
	...
	/** Returns the current rows-per-page value. */
	public int getRowsPerPage() {
		return parseRowsPerPage(rowsPerPageInput.getText());
	}
	...
	private HBox createCenterSection() {
		...
		HBox centerBox = new HBox(INNER_SPACING, firstButton, prevButton, pagePrefix, pageInput, pageIndicatorLabel,
	...
	/**
	 * Handles commit action for the page input field. Validates and navigates to
	 * the entered page number.
	 */
	...
	/**
	 * Handles commit action for the rows-per-page input field. Validates and stores
	 * the value or reverts to the last valid value.
	 */
	...
	private int parseRowsPerPage(String text) {
		if (!isValidRowsPerPage(text)) {
			return Integer.parseInt(DEFAULT_ROWS_PER_PAGE);
		}
