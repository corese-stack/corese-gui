...
/**
 * Manages the application's core data state and query operations.
 * This class is a Singleton and acts as the primary access point for the
 * main data graph, query execution, result caching, and data loading.
 * This is the full and complete replacement for the old ApplicationStateManager.
 */
public class QueryManager {
    ...
    public TabCacheEntry getCachedResult(Integer tabId) { return queryResultCache.get(tabId); }

    public void clearCacheForTab(Integer tabId) { queryResultCache.remove(tabId); }
    ...
    public String getFormattedCachedQuery(Integer tabId, String format) {
        ...
        if (cachedEntry.getMappingsResult() != null) {
            coreseFormat = switch (upperCaseFormat) {
                case "XML" -> ResultFormat.format.XML_FORMAT; case "JSON" -> ResultFormat.format.JSON_FORMAT;
                case "CSV" -> ResultFormat.format.CSV_FORMAT; case "TSV" -> ResultFormat.format.TSV_FORMAT;
                case "MARKDOWN" -> ResultFormat.format.MARKDOWN_FORMAT; default -> null;
            };
            return (coreseFormat != null) ? formatMappings(cachedEntry.getMappingsResult(), coreseFormat) : "Unsupported format for SELECT/ASK.";
        }
        if (cachedEntry.getGraphResult() != null) {
            coreseFormat = switch (upperCaseFormat) {
                case "TURTLE" -> ResultFormat.format.TURTLE_FORMAT; case "RDF_XML" -> ResultFormat.format.RDF_XML_FORMAT;
                case "JSON_LD" -> ResultFormat.format.JSONLD_FORMAT; case "N_TRIPLES" -> ResultFormat.format.NTRIPLES_FORMAT;
                case "N_QUADS" -> ResultFormat.format.NQUADS_FORMAT; case "TRIG" -> ResultFormat.format.TRIG_FORMAT;
                default -> null;
            };
            return (coreseFormat != null) ? formatGraph(cachedEntry.getGraphResult(), coreseFormat) : "Unsupported format for CONSTRUCT/DESCRIBE.";
        }
        return "";
    }
    ...
    public FileListModel getFileListModel() { return fileListModel; }
    public List<File> getLoadedFiles() { return new ArrayList<>(loadedFiles); }
    public int getTripletCount() { return (this.graph != null) ? this.graph.size() : 0; }
    public int getSemanticElementsCount() { return getTripletCount(); }
    public int getGraphCount() { return 1; }
    public String getLastGraphResult() { return this.lastGraphResult; }
    ...
    public static class TabCacheEntry {
        ...
        public TabCacheEntry(String queryType, Mappings mappings) { this.queryType = queryType; this.mappingsResult = mappings; this.graphResult = null; }
        public TabCacheEntry(String queryType, Graph graph) { this.queryType = queryType; this.mappingsResult = null; this.graphResult = graph; }
