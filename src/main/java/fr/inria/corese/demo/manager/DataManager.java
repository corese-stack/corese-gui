...
public class DataManager {
  ...
  /**
   * Manages RDF data and associated files for the application using a singleton pattern.
   * Responsibilities include: - Loading RDF files (.ttl, .rdf/.xml, .jsonld) into the shared Corese
   * Graph. - Tracking loaded files via FileListModel and maintaining an in-memory list. - Reloading
   * all files after graph reinitialization or file removal. - Saving the current graph as Turtle
   * (.ttl). - Saving/restoring a snapshot of the loaded-files state. - Clearing the graph and file
   * lists, and providing the content of the last loaded file. *
   */
  ...
  public synchronized void saveCurrentState() {
    ...
    queryManager.addLogEntry(
        "Current state snapshot saved (" + savedLoadedFiles.size() + " files).");
  }
